...
import { SecurityLockGate } from './components/SecurityLockGate';
...
export default function App() {
  ...
  // Load saved simulations from localStorage on mount & subscribe to cloud security
  useEffect(() => {
    try {
    ...
    // Subscribe to cloud security settings in real-time
    const unsubscribeSecurity = subscribeToSecuritySettings();
    return () => {
      unsubscribeSecurity();
    };
  }, []);
...
★ 액면가 대비 배수: ${calculations.faceMultiple.toFixed(2)}배 (자본금 ${formatKoreanUnit(calculations.capitalStock)})`;
  ...
  if (!isAuthenticated) {
    return <SecurityLockGate onUnlock={() => setIsAuthenticated(true)} />;
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 font-sans flex flex-col justify-between print:bg-white print:pb-0">
      ...
      <header className="px-3.5 sm:px-8 py-2.5 sm:py-3.5 bg-[#1E293B] text-white border-b-4 border-[#F37321] sticky top-0 z-30 shadow-md no-print">
        <div className="w-full max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5 sm:gap-4">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            {/* Brand Emblem (Official Hanwha Logo - Click for Admin Security) */}
            <button
              type="button"
              onClick={() => setShowSecurityModal(true)}
              className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg overflow-hidden shadow-sm shrink-0 border border-orange-400/40 hover:border-orange-300 hover:ring-2 hover:ring-orange-400/30 bg-[#F37321] flex items-center justify-center cursor-pointer transition active:scale-95 focus:outline-none"
              title="관리자 보안 통제 및 설정"
            >
              <img
                src="/logo.svg"
                alt="한화 로고"
      ...
      {/* ── MODAL 5: SECURITY & PASSWORD CHANGE MODAL ── */}
      <PasswordChangeModal
        isOpen={showSecurityModal}
        onClose={() => setShowSecurityModal(false)}
        onLogout={() => {
          setIsAuthenticated(false);
          showToast('화면이 안전하게 잠겼습니다.');
        }}
